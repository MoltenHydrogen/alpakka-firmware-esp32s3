...
void hid_press_multiple_delayed(esp_timer_handle_t alarm, uint8_t *keys) {
    esp_timer_stop(alarm);
    esp_timer_delete(alarm);
    for(uint8_t i=0; i<4; i++) {
        if (keys[i] == 0) return;
        hid_press(keys[i]);
    }
...
void hid_release_multiple_delayed(esp_timer_handle_t alarm, uint8_t *keys) {
    esp_timer_stop(alarm);
    esp_timer_delete(alarm);
    for(uint8_t i=0; i<4; i++) {
        if (keys[i] == 0) return;
...
void hid_thanks_() {
    ...
    .callback = hid_thanks_,
    .arg = NULL,
    .dispatch_method = ESP_TIMER_TASK,
    .skip_unhandled_events = false
    };
    esp_timer_create(&alarm_cfg, &alarm);
    esp_timer_start_once(alarm, 5*1000);
}
